...
[A] CONFLICT PIPELINE  (rule 7: the local layer first, this node only after)
    ...
    escalated to a yield:                            3

      #  pair               outcome           age    min sep    gain
      1  amr1/amr2          escalated          2.0s     1.16m   -0.67m
      2  amr1/amr2          escalated          2.0s     0.86m   -0.71m
      3  amr1/amr2          escalated          2.0s     0.86m   +0.00m
...
[B] YIELDS COMMANDED
      ...
      1  amr2   amr1          1.0s   conflict cleared        1.16m   1.16m
      2  amr2   amr1         45.0s   max hold elapsed (fail-safe)  0.86m   0.65m
      3  amr2   amr1          4.7s   conflict cleared        0.86m   0.85m

    yield 1: escalated 2.0 s after the conflict was first predicted, at t = 42.5 s
             21 zero-twist commands published on amr2/cmd_vel_yield (mux priority 150)
             released at t = 43.5 s on: conflict cleared
    yield 2: escalated 2.0 s after the conflict was first predicted, at t = 46.5 s
             901 zero-twist commands published on amr2/cmd_vel_yield (mux priority 150)
             released at t = 91.5 s on: max hold elapsed (fail-safe)
    yield 3: escalated 2.0 s after the conflict was first predicted, at t = 93.5 s
             95 zero-twist commands published on amr2/cmd_vel_yield (mux priority 150)
             released at t = 98.3 s on: conflict cleared
...
[C] NAV2 RECOVERY DURING THE HOLD  (ENGINEERING_NOTES rule 2)
    ...
    progress_checker.movement_time_allowance, read back from controller_server:
      ...
      yield 3 (amr2): at entry    1000000 s   after release         10 s
...
[D] SAFETY GATE STATE DURING THE HOLD  (is this a yield or a halt?)
    ...
    yield 2 (amr2): SafetyGate was blocking on 0 of 901 held cycles (0 %)
    yield 3 (amr2): SafetyGate was blocking on 0 of 95 held cycles (0 %)
...
[E] VERDICT
    ...
    no hold ended on the fail-safe ceiling:              NO
