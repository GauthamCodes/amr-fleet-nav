# Phase 3 - concurrent goals, CURRENT behaviour

**This is the run that reproduces today, on the shipped configuration.** It is kept
beside `phase3_concurrent_goals.md`, which records both robots succeeding and is
**historical** - that artifact predates the payload motion chain and the fleet
trajectory layer, and no run since reproduces it.

Both goals are dispatched together, both are accepted, and both plans are published in
`fleet_map`. **Only amr1 arrives.** amr2 aborts on `Failed to make progress` having
driven 0.07 m. Written up in README section 10.
...
[A] PER ROBOT
      ...
      amr1         SUCCEEDED       18.7     10.50    10.52      0.020         0
      amr2         ABORTED          0.5     10.50     0.07     10.433         0
...
[C] GLOBAL COSTMAP AND THE RAMP FILTER
      ...
      amr2         fleet_map    680x400     272000  252329    19671          0
      amr1         fleet_map    680x400     272000  251224    20776          0
...
[D] VERDICT
    ...
    every robot reached its goal:                      NO
