...
again on the same commit with **no source change**, both robots arrive, four runs out
of four:
...
All four ran `phase3_fleet_goals.launch.py` at its defaults. Runs 1 and 3 had the Gazebo
GUI and RViz attached; runs 2 and 4 were headless. Every run reports 0 replans.
...
What these four runs support is only this: **the code in this repository reaches both
