# PHASE 7 - forced narrow-intersection conflict, escalated to a yield  [CONTROL: no suppression]
...
[A] CONFLICT PIPELINE  (rule 7: the local layer first, this node only after)
    ...
    resolved WITHOUT escalation:                     2
    escalated to a yield:                            1

      #  pair               outcome           age    min sep    gain
      1  amr1/amr2          escalated          2.0s     1.37m   -0.29m
      2  amr1/amr2          resolved locally   1.6s     1.93m   +0.03m
      3  amr1/amr2          resolved locally   0.4s     1.91m   +0.05m
...
[B] YIELDS COMMANDED
      ...
      1  amr2   amr1          1.0s   conflict cleared        1.42m   1.42m

    yield 1: escalated 2.0 s after the conflict was first predicted, at t = 38.2 s
             21 zero-twist commands published on amr2/cmd_vel_yield (mux priority 150)
             released at t = 39.2 s on: conflict cleared
...
[C] NAV2 RECOVERY DURING THE HOLD  (docs/ENGINEERING_NOTES.md rule 2)
    ...
    progress_checker.movement_time_allowance, read back from controller_server:
      yield 1 (amr2): at entry          n/a   after release          n/a
